...
public class Tables implements Serializable {
    ...
    public String reserve(Branches b, String name, int num, int num1, int num2, Customer c1) throws IOException {
        ...
        for (int i = num1; i < num2; i++) {
            if (b.branches.get(name).get(i).get(num - 1).reservation == false) {
                count++;
            }
        }
        if (count == (num2 - num1)) {
            ...
            try {
                FileWriter fr = new FileWriter(bfile, true);
                BufferedWriter br = new BufferedWriter(fr);
                PrintWriter pr = new PrintWriter(br);
            ...
            return "Table " + num + " is already reserved in that time";
        }
    }
    ...
    String reservedTables(Branches b, String name, int num, int num1, int num2, String str) {
        ...
        for (int j = 0; j < b.branches.get(name).get(num1).size(); j++) {
            int count = 0;
            for (int i = num1; i < num2; i++) {
                if (b.branches.get(name).get(i).get(j).reservation == false) {
                    count++;
                }
            }
            if (count == (num2 - num1)) {
